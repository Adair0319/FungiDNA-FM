#!/usr/bin/env python3
"""Build independent Phylum and Subphylum 10Kbp datasets.

Data source: 1kfg_datasets/assembly_genome/*.fasta (735 raw genomes)
Output: data/downstream/task0_phylum/ and task0_subphylum/
"""
...
def build_dataset(
    ...
    print(f"Labels: {len(label_map)} classes: {list(label_map.keys())}")
    ...
    with open(output_dir / "ignore_labels.json", "w") as f:
        # Store as list of label indices for the training/eval scripts
...
def main():
    ...
    # ── Phylum dataset ──
    build_dataset(
        task_name="task0_phylum",
        rank="phylum",
        exclude_labels=None,
        output_dir=OUTPUT_BASE / "task0_phylum",
        rng=rng,
    )

    # ── Subphylum dataset ──
    build_dataset(
        task_name="task0_subphylum",
        rank="subphylum",
        exclude_labels={"Incertae sedis"},
        output_dir=OUTPUT_BASE / "task0_subphylum",
        rng=rng,
    )

    print(f"\n{'='*60}")
    print("Done. Outputs:")
    print(f"  {OUTPUT_BASE / 'task0_phylum'}")
    print(f"  {OUTPUT_BASE / 'task0_subphylum'}")
    print(f"{'='*60}")
